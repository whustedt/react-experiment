...
import { z } from 'zod';
import { BasketScope, type DomainObjectType, WorkItemStatus } from '../../../api/workItems';
import {
  DOMAIN_OBJECT_LABEL_MAP,
  WORK_ITEM_STATUS_COLOR_MAP,
  WORK_ITEM_STATUS_OPTIONS,
} from '../../../shared/constants/workItemDisplay';
...
export function WorklistPage() {
  const [basket, setBasket] = React.useState(BasketScope.MY);
  ...
  const columns: GridColDef[] = [
    ...
    { field: 'objectLabel', headerName: 'Fachobjekt', flex: 1.1 },
    {
      ...
      headerName: 'Typ',
      flex: 0.8,
      valueFormatter: (value: DomainObjectType) => DOMAIN_OBJECT_LABEL_MAP[value] ?? value,
    },
    {
      field: 'status',
      headerName: 'Status',
      ...
      renderCell: (params) => (
        <Chip
          label={params.value}
          color={WORK_ITEM_STATUS_COLOR_MAP[params.value as WorkItemStatus]}
          size="small"
        />
      ),
    },
            ...
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2} alignItems={{ md: 'center' }}>
              ...
              {basket === BasketScope.COLLEAGUE && (
                <Controller
                  ...
                  control={basketForm.control}
                  render={({ field }) => (
                    <TextField {...field} label="Kollege" placeholder="z. B. Bob" sx={{ minWidth: 240 }} />
                  )}
                />
              )}
          ...
          >
            <Stack direction={{ xs: 'column', md: 'row' }} spacing={2}>
              ...
              <Controller
                ...
                control={globalForm.control}
                render={({ field }) => (
                  <TextField {...field} select label="Status" sx={{ minWidth: 220, bgcolor: '#fff', borderRadius: 2 }}>
                    <MenuItem value="">Alle</MenuItem>
                    {WORK_ITEM_STATUS_OPTIONS.map((status) => (
                      <MenuItem key={status} value={status}>
                        {status}

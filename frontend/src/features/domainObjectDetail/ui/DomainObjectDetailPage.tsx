...
import { DOMAIN_OBJECT_LABEL_MAP } from '../../../shared/constants/workItemDisplay';
...
export function DomainObjectDetailPage() {
  ...
  return (
    <Stack spacing={3}>
      ...
      <Card sx={{ borderRadius: 4, background: 'linear-gradient(135deg, #0f172a, #1e3a8a)', color: '#fff' }}>
        <CardContent>
          <Typography variant="h4" fontWeight={700}>
            {DOMAIN_OBJECT_LABEL_MAP[objectType]} · {contextQuery.data.title}
          </Typography>
          <Typography sx={{ opacity: 0.85 }}>{contextQuery.data.subtitle}</Typography>
        </CardContent>
      </Card>
          ...
          {tab === 1 && (
            <Stack spacing={2} mt={2}>
              ...
              {uploadMutation.isSuccess && (
                <Alert severity="success">Dokument wurde hochgeladen und indiziert.</Alert>
              )}
            </Stack>
          )}
